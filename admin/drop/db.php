...
if(isset($_POST['ok'])){
	...
	}else if($drop == 't_n_rapor'){
		$truncate = "TRUNCATE TABLE t_n_rapor";
		mysqli_query($conn, $truncate);

		echo "<script>
				alert('Tabel Rapor Sudah Kosong');
				window.location.href='?page=".base64_encode("db")."';
			</script>";
	}else if($drop == 't_n_sekolah'){
		$truncate = "TRUNCATE TABLE t_n_sekolah";
		mysqli_query($conn, $truncate);

		echo "<script>
				alert('Tabel USBN Sudah Kosong');
				window.location.href='?page=".base64_encode("db")."';
			</script>";
	}
}
...
<fieldset>
	...
	<form action="" method="post" style="width: 30%; text-align: justify;">
		...
		<div class="form-group">
			<div class="checkbox">
                <label>
                    <input type="checkbox" name="drop" value="t_n_rapor"><b>Kosongkan Tabel Nilai Rapor</b>
                </label>
            </div>
		</div>
		<div class="form-group">
			<div class="checkbox">
                <label>
                    <input type="checkbox" name="drop" value="t_n_sekolah"><b>Kosongkan Tabel USBN</b>
                </label>
            </div>
		</div>
		...
		<a href="beranda.php" class="btn btn-warning btn-sm">
