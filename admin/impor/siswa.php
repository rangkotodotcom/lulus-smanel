	...
	if(isset($_POST['preview'])){
		...
		if($tipe_file == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"){
			...
			foreach ($sheet as $row) {
						...
						if(empty($nama) &&  empty($t_lahir) &&  empty($tgl_lhr) &&  empty($n_ortu) &&  empty($nis) &&  empty($nisn) &&  empty($no_pes) &&  empty($kelas) &&  empty($foto)) continue;
						...
						if($numrow > 1){
							...
							if(empty($nama) or empty($t_lahir) or empty($tgl_lhr) or empty($n_ortu) or empty($nis) or empty($nisn) or empty($no_pes) or empty($kelas) or empty($foto)){
								$kosong++;
							}
