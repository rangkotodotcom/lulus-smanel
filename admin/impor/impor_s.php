...
if(isset($_POST['import'])){
	...
	foreach($sheet as $row){
		...
		if(empty($nama) && empty($t_lahir) && empty($tgl_lhr) && empty($n_ortu) && empty($nis) && empty($nisn) && empty($no_pes) && empty($kelas) && empty($foto))
            continue; 
                        
		
		if($numrow > 1){
			// Tambahkan value yang akan di insert ke variabel $query
			$query .= "('".$nama."','".$t_lahir."','".$tgl_lhr."','".$n_ortu."','".$nis."','".$nisn."','".$no_pes."','".$kelas."','".$foto."'),";
		}
