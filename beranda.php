...
$cek = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM t_luser WHERE user = '$kd' " ));

if($cek['email'] == ''){
	echo "<script>window.location.href= 'email/?user=".base64_encode($_SESSION['user'])."'</script>";
}

if($cek['ganti'] != 'sudah'){
	if($_SESSION['level'] == 'siswa'){
		echo "<script>window.location.href= 'ganti/siswa/?user=".base64_encode($_SESSION['user'])."'</script>";
	}else{
		echo "<script>window.location.href= 'ganti/admin/?user=".base64_encode($_SESSION['user'])."'</script>";
	}
}
...
<html>
<head>
	<title><?= $_SESSION['nama'] ?> | Pengumuman Kelulusan SMAN 1 ENAM LINGKUNG </title>
...
<body style="background-image: url(img/tetap/dark.png);">
	<div style="padding: 20px 0;" class="container">
		<center>
			<div class="header">
				<h4><marquee>Portal Pengumuman Kelulusan SMAN 1 ENAM LINGKUNG</marquee></h4>
			</div>
			<div class="content cf">
					<div class="sidebar">
						<div class="profil">
							...
							?>
								...
								<img src="img/tetap/profil.jpg" class="img-circle" alt="Foto Belum Ada" width="100px" /><br>
							
							<?php }else{ ?>

							<img src="img/siswa/<?= $data['foto']; ?>" class="img-circle" alt="Foto Belum Ada" width="100px" height="109px" /><br>
						...
						<div class="menu">
							...
							<a href="?page=<?= base64_encode("rapor_usbn"); ?>&user=<?= base64_encode($_SESSION['user']);?>">Rapor & USBN
							</a><br>
							<a href="?page=<?= base64_encode("upload_foto"); ?>&user=<?= base64_encode($_SESSION['user']);?>">Upload Foto
							</a><br>
							...
							<a href="?page=<?= base64_encode("nilai_us"); ?>">Nilai Sekolah</a><br>
							...
							<a href="?page=<?= base64_encode("akses"); ?>">System</a>
					...
					<div class="main">
						...
						if($level){
							...
							}else if($page == 'rapor_usbn'){
								include 'siswa/rapor_usbn.php';
							}else if($page == 'upload_foto'){
								include 'siswa/upload_foto.php';
							}
						}else{
							if($page == 'd_siswa'){
								if($action == ""){
